...
//User should derive from DbContext and then he will add wanted DbSet from entities.We need to discover run-time added DbSets from users.
public class DbContext
{
    ...
    public void SaveChanges()
    {
        ...
        {
            IEnumerable<object> invalidEntities = dbSet.Where(e => !this.IsObjectValid(e)).ToArray();
            if (invalidEntities.Any())
            {
                throw new InvalidOperationException(String.Format
                    (ExceptionMessages.InvalidEntitiesException, invalidEntities.Count(), dbSet.GetType().Name));
            }
        }
        ...
        where TEntity : class, new()
    {
        ...
        IEnumerable<TEntity> fetchesRows = this.connection
    ...
    private string[] GetEntityColumnNames(Type entityType)
    {
        ...
        string[] columnsTaken = entityType.GetProperties()
        ...
        where TEntity : class, new()
    {
        ...
        {
            Type collectionEntityType = collection.PropertyType.GetGenericArguments().First();
        ...
        where TEntity : class, new()
    {
        ...
        foreach (PropertyInfo foreignKey in foreignKeys) 
        {
            ...
            IEnumerable<object> navigationDbSet = (IEnumerable<object>)this.dbSetProperties[navigationProperty.PropertyType].GetValue(this);
            ...
            foreach (TEntity entity in dbSet)
            {
                ...
                object navigationPropertyValue = navigationDbSet.First(cnp => navigationEntityPK.GetValue(cnp).Equals(foreignKeyValue));
        ...
        where TCollection : class, new()
    {
        ...
        PropertyInfo[] collectionTypePrimaryKeys = collectionType.GetProperties()
                                                                 .Where(pi => pi.HasAttribute<KeyAttribute>())
                                                                 .ToArray();
        PropertyInfo foreignKey = collectionTypePrimaryKeys.First();
        PropertyInfo primaryKey = entityType.GetProperties()
                                            .First(pi => pi.HasAttribute<KeyAttribute>());

        bool isManyToMany = collectionTypePrimaryKeys.Length >= 2;
        if (isManyToMany)
        {
            foreignKey = collectionType.GetProperties()
